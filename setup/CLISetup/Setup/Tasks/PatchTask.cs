...
namespace Setup.Tasks {
	public class PatchTask : SetupOperation {
		...
		public static IEnumerable<(string file, string relPath)> EnumerateSrcFiles(string dir) =>
			FileHandler.EnumerateFiles(dir).Where(f => !f.relPath.Split('/', '\\').Any(nonSourceDirs.Contains));
		...
		public PatchTask(ITaskInterface taskInterface, string baseDir, string patchedDir, string patchDir, ProgramSetting<DateTime> cutoff) : base(taskInterface) {
			this.baseDir = FileHandler.PreparePath(baseDir);
			this.patchedDir = FileHandler.PreparePath(patchedDir);
			this.patchDir = FileHandler.PreparePath(patchDir);
			this.cutoff = cutoff;
		}

		public override bool StartupWarning() {
			taskInterface.SelectDialog<bool>($"Any changes in /{patchedDir} that have not been converted to patches will be lost. Continue?", out bool input);
			return input;
		}

		public override void Run() {
			FileHandler.Config config = FileHandler.Config.GetConfig();
			FileHandler.UpdateTargetsFiles(); //Update branch information

			mode = (Patcher.Mode)config.PatchMode;
			...
			foreach (var (file, relPath) in FileHandler.EnumerateFiles(patchDir)) {
				if (relPath.EndsWith(".patch")) {
					items.Add(new WorkItem("Patching: " + relPath, () => newFiles.Add(FileHandler.PreparePath(Patch(file).PatchedPath))));
					...
					items.Add(new WorkItem("Copying: " + relPath, () => FileHandler.Copy(file, destination)));
			...
			try {
				FileHandler.CreateDirectory(FileHandler.logsDir);
				logFile = new StreamWriter(Path.Combine(FileHandler.logsDir, "patch.log"));
			...
			FileHandler.DeleteEmptyDirs(patchedDir);

			//Show patch reviewer if there were any fuzzy patches.

			// Patch reviewer also needs to be rewritten so anything referencing it doesn't get to exist
		...
		public override void FinishedDialog() {
			...
			taskInterface.Message($"Patches applied with {failures} failures and {warnings} warnings.\nSee /logs/patch.log for details.");
		}
		...
		private FilePatcher Patch(string patchPath) {
			...
			patcher.Patch(mode);
			results.Add(patcher);
			FileHandler.CreateParentDirectory(patcher.PatchedPath);
			patcher.Save();
